        ...
        if (xQueueReceive(nvs_save_queue, &dummy, pdMS_TO_TICKS(1000)) == pdTRUE) {
            ...
            if ((now - last_save_time) >= 1000) {
                ...
                if (err == ESP_OK) {
                    ...
                    ESP_LOGI(TAG, "State saved (Mode:%d, Work:%lu, Idle:%lu, Planned:%lu)", 
                             current_mode, sys_data.work_time, sys_data.idle_time, sys_data.planned_time);
                    last_save_time = now;
                }
            }
        }
...
void nvs_storage_save_target(uint32_t target) {
    ...
    if (err == ESP_OK) {
        ...
        nvs_commit(my_handle);
        nvs_close(my_handle);
        ESP_LOGI(TAG, "Target saved: %lu", target);
    }
}
...
uint32_t nvs_storage_load_target(void) {
    ...
    ESP_LOGI(TAG, "Target loaded: %lu", target);
    return target;
}
...
void nvs_storage_save_cycle_target(uint32_t seconds) {
    ...
    if (err == ESP_OK) {
        ...
        nvs_commit(my_handle);
        nvs_close(my_handle);
        ESP_LOGI(TAG, "Cycle target saved: %lu sec", seconds);
    }
}
...
uint32_t nvs_storage_load_cycle_target(void) {
    ...
    ESP_LOGI(TAG, "Cycle target loaded: %lu sec", seconds);
    return seconds;
}
...
system_state_backup_t nvs_storage_load_state(void) {
    ...
    esp_err_t err = nvs_open("storage", NVS_READONLY, &my_handle);
    if (err == ESP_OK) {
        nvs_get_u8(my_handle, "work_mode", &state.work_mode);
        ...
        nvs_close(my_handle);
        ESP_LOGI(TAG, "State loaded (Mode:%d, Work:%lu, Idle:%lu, Planned:%lu)", 
                 state.work_mode, state.work_t, state.idle_t, state.planned_t);
    }
    return state;
}
...
void nvs_storage_save_state_immediate(void) {
    ...
    if (err == ESP_OK) {
        ...
        nvs_commit(my_handle);
        nvs_close(my_handle);
        ESP_LOGI(TAG, "State saved immediately");
    }
}
